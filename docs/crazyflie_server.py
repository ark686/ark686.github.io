...
class CrazyflieServer(Node):
    def __init__(self):
        ...
        self.default_log_vars = {"pose": ['stateEstimate.x', 'stateEstimate.y', 'stateEstimate.z',
                                 ...
                                 "scan": ['range.front', 'range.left', 'range.back', 'range.right', 'range.up'],
    ...
    def _log_scan_data_callback(self, timestamp, data, logconf, uri):
        ...
        up_range = float(data.get('range.up'))/1000.0
        ...
        if up_range > max_range:
            up_range = float("inf")
        self.ranges = [back_range, right_range, front_range, left_range, up_range]
